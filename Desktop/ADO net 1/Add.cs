    ...
    public partial class Add : Form
    {
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            using (cn = new NpgsqlConnection(connectionString))
            {
                ...
                {
                    cn.Open();
                    string strSQL = "SELECT good.picture, good.price FROM good INNER JOIN supply ON good.idgood = supply.idgood INNER JOIN provider ON supply.provider = provider.provider WHERE good.name = '" + comboBox2.Text + "'";
                    NpgsqlCommand command = new NpgsqlCommand(strSQL, cn);
                    NpgsqlDataReader rdr = command.ExecuteReader();
                    while (rdr.Read())
                    {
        ...
        private void numericUpDown1_ValueChanged(object sender, EventArgs e)
        {
            using (cn = new NpgsqlConnection(connectionString))
            {
                ...
                {
                    cn.Open();
                    string strSQL = "SELECT good.price FROM good INNER JOIN supply ON good.idgood = supply.idgood INNER JOIN provider ON supply.provider = provider.provider WHERE good.name = '" + comboBox2.Text + "'";
                    NpgsqlCommand command = new NpgsqlCommand(strSQL, cn);
                    NpgsqlDataReader rdr = command.ExecuteReader();
                    while (rdr.Read())
                    {
